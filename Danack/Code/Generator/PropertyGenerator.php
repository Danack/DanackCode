...
use Danack\Code\Reflection\PropertyReflection;
...
class PropertyGenerator extends AbstractMemberGenerator
{
    ...
    public function setStandardDocBlock($type = null, $description = null) 
    {
        $name = $this->getName();
        $docBlockText = '';

        if ($description != null) {
            $docBlockText .= $description;
            $docBlockText .= "\n";
        }

        if ($type == 'array' || $type === 'callable' || $type == 'string' || $type == 'bool' || $type === null) {
